...
#include "ntdddisk.h"
#include "wdmsec.h"
#include "FileFilterAndMonitor.h"

/**
 * 文件系统过滤与监控
 * 
 * 文件创建、打开、读写、重命名；目录创建、打开、枚举、重命名、删除等
 * 可以实现诸如检查病毒、数据加密、数据备份、安全监控等功能
 * 比串口、键盘、硬盘等过滤驱动更为复杂
 * 需要注意文件系统驱动与存储驱动的区别，硬盘就是典型的存储设备，负责数据读写
 * 而文件系统负责将数据在硬盘的空间中组织成文件和目录
 * 文件和目录都是复杂的信息集合，包含各种属性，目录又可以嵌套，对目录操作就涉及子目录和文件操作
 * 
 * 文件过滤应用于实时监控的防病毒软件十分典型，在任何操作试图写一个文件时，防病毒软件都会过滤写入内容，检查是否含有病毒特征码
 * 
 * Windows上常见的文件系统为FAT32和NTFS，驱动分别为fastfat.sys和ntfs.sys
 * 虽然复杂，但基本思路还是生成过滤设备对象来绑定真实设备对象
 * 文件系统一般生成两类设备，首先是控制设备，主要任务是修改整个驱动的内部配置；另一种是卷设备，一个卷对应一个逻辑盘
 * 一个文件系统可能有多个加载的卷，类似"C:"这样的盘符是设备的符号链接，真正的设备名一般是\Device\harddiskVolume这样
 * 当一个卷使用了某种文件系统，该文件系统会为该设备生成一个设备对象
 * 过滤是绑定的就是文件系统的卷设备对象，而不是真实的卷设备
 * 而绑定文件系统卷设备的前提是绑定文件系统的控制设备
 * 
 * 
 * 以学习WDK目录下\src\filesys\filter\sfilter代码为例
 * 并修改为一个lib项目，将接口封装给其他程序调用，就可以基于官方例子快速开发所需功能了
 */
...
/**
 * 需要生成驱动自身的CDO，因为文件过滤驱动往往需要和带界面的用户应用通信，将发现的信息交给上层
 * 通信接口就是这个控制设备
 */
NTSTATUS
DriverEntry(
