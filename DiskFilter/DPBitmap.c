 ...
 * 位图操作
 * 实际上是一个内存块
 * 这些内存块每一位用来标识一个磁盘上的最小访问单位
 * 一般情况下是一个扇区
 * 每一位可以被置位或者被清除，用来表示扇区对应的两种状态
 * 作为一个还原驱动，关键在于如何将写入数据存储到其他地方，读取时又能准确从其他地方找到
 * bitmap中每一位对应磁盘上一个扇区，有多少扇区就有多少位
 * 位0代表此位对应扇区数据没有被存储到其他地方
 * 写数据时，根据操作范围可将bitmap对应的区域置为1
 * 读操作时，会根据bitmap内容把置1的扇区从转存区读回，对位0的地方从原设备读取
 */
