 ...
 * 磁盘过滤驱动
 * 
 * 过滤的一般操作就是在设备栈中加入自己的设备，Windows发送的IRP请求会先发给设备栈中最上层设备之后依次传递
 * 实际应用中，一些即插即用设备以及相同类型的设备，需要一种类过滤驱动，能够在某一特定设备建立时由PnP管理器调用指定的过滤驱动代码并绑定
 * 分为上层过滤和下层过滤，上层设备位于实际功能设备之上，会先获得系统发来的IRP请求，最为常见
 * 
 * Windows中，由磁盘设备和磁盘卷设备两种类型的设备
 * 若磁盘卷位于某个磁盘上，那么对磁盘卷访问最终也会体现在磁盘上，但并不一定在同一个设备栈上
 * 驱动角度，两种设备读写请求都针对磁盘大小或卷大小范围内，都以扇区对齐
 * 
 * 普通驱动程序可以通过注册表将自身声明为类过滤驱动
 * 
 * 本例将实现一个磁盘还原操作的驱动，网吧或学校机房应该使用的就是类似技术，每次重启时都恢复到最初状态
 * （书中限制很多，学习代码，并没尝试，需要使用OsrLoader程序）
 * 简单的实现思路是：
 * 开启还原后，对还原卷的写操作被移到其他地方（转存处），不会真正的写入该卷
 * 对于读操作，原本存在的还正常读，开启功能后写入的会从转存处把之前写的内容读出来
 * 读写需要互斥，重启后转存处数据清零，就起到了重启后还原的效果
 */
...
)
{
	...
	//本驱动作为boot类型驱动（在注册表中指定）存在，此类驱动是启动最早的驱动程序，在系统引导时加载完毕
